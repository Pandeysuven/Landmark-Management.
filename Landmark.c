...
int AddLandmark(char *file_location, int area, int lmark_type)
{
	...
	{
		if(strcmpi(lmark_b.address, lmark.address) == STR_MATCH && search_by_name(lmark.name, lmark_type) == FOUND)
		{
